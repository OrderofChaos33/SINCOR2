...
# 8. Crypto-launch promotion patterns (added 2026-05-18 for SINC relaunch)
#
# Each pattern targets the COMBINATION of token-context + financial-promise
# language. Matching just "100x" or "moon" alone is too broad; matching them
# next to a token/price context catches the actual risk: implied securities
# promotion, federal wire-fraud exposure (DOJ Mango/EthereumMax precedent).
CRYPTO_PROMOTION_PATTERNS = [
    (r"\$\d+(\.\d+)?\s+(SINC|AXM|sinc|axm)\b", "Token price target"),
    (r"\b(SINC|AXM|sinc|axm)\s+(will|going|gonna|expect)(\s+to)?\s+(\$|\d)", "Token price prediction"),
    (r"\b(reach|hit|pump|moon)\s+(to\s+)?\$\d", "Price target"),
    (r"\b\d+x\s+(gain|return|potential|target|opportunity|profit)\b", "Multiplier gain claim"),
    (r"\bgains?\s+of\s+\d+x\b", "Multiplier gain claim"),
    (r"\bto\s+the\s+moon\b", "Moonshot speculation"),
    (r"\bmoonshot\b", "Moonshot speculation"),
    (r"\bnext\s+(bitcoin|ethereum|solana|btc|eth|sol)\b", "Next-Bitcoin-style hype"),
    (r"\b(gem|alpha)\s+(alert|found|coming|incoming)\b", "Gem/alpha speculation"),
    (r"\bape\s+in\b", "Ape-in pump language"),
    (r"\b(binance|coinbase|kraken|okx)\s+listing\s+(coming|incoming|next|soon|imminent)\b", "Promised CEX listing"),
    (r"\b(don'?t\s+miss|last\s+chance)\s+(out\s+)?(on\s+)?(this\s+)?(token|launch|gem|opportunity)\b", "FOMO inducement"),
    (r"\bguaranteed?\s+(pump|gains?|return|profit|moon)\b", "Guaranteed return/pump"),
    (r"\b(everyone|all)\s+buy(\s+now)?\b", "Coordinated-buy call"),
    (r"\bcoordinated\s+buy\b", "Coordinated-buy call"),
    (r"\bsend\s+it\b\s*(token|coin|launch|degen)", "Pump-speculation language"),
]


# 9. Acceptable AI-disclosure variants per FTC Endorsement Guides (2023 update).
# Promotional content tagged as crypto-launch MUST contain at least one of these.
AI_DISCLOSURE_VARIANTS = [
    "ai-generated",
    "ai generated",
    "generated by ai",
    "generated by sincor",
    "posted by sincor agent",
    "drafted by ai",
    "ai-assisted",
    "ai-authored",
    "written by ai",
...
class GuardrailsEngine:
    ...
    # ------------------------------------------------------------------
    # Crypto-launch promotion (added 2026-05-18 for SINC relaunch)
    # ------------------------------------------------------------------

    def check_crypto_promotion(self, content: str, agent: str = "unknown") -> None:
        """Block crypto-launch content that implies price targets, multiplier
        returns, moonshot speculation, FOMO inducement, or coordinated buying.
        Each match is a securities-promotion / wire-fraud red flag (DOJ
        Mango/EthereumMax precedent; SEC unregistered-securities promotion).
        """
        for pattern, description in CRYPTO_PROMOTION_PATTERNS:
            if re.search(pattern, content, re.IGNORECASE):
                self._enforce(
                    rule="NO_CRYPTO_PRICE_PROMOTION",
                    agent=agent,
                    action="publish_content",
                    reason=f"Content contains prohibited crypto-promotion pattern: '{description}' (matched: {pattern})",
                    remediation=(
                        "Crypto launch content must be factually grounded in on-chain "
                        "data only — no price targets, no multiplier returns, no "
                        "'to the moon', no coordinated-buy calls. Rewrite using "
                        "verifiable facts (contract address, audit score, holder "
                        "count, burn count). Reference DOJ U.S. v. Eisenberg 2024 "
                        "and SEC unregistered-securities-promotion precedent. "
                        "See [[launch_tactics_legal_line]] memory for the project "
                        "constraint the user accepted."
                    ),
                )

    def check_ai_disclosure(self, content: str, agent: str = "unknown") -> None:
        """Promotional content tagged crypto-launch MUST contain an AI
        disclosure string per FTC Endorsement Guides (2023 update). Absence
        of any acceptable disclosure variant is a hard block.
        """
        content_lower = content.lower()
        if not any(variant in content_lower for variant in AI_DISCLOSURE_VARIANTS):
            self._enforce(
                rule="REQUIRE_AI_DISCLOSURE",
                agent=agent,
                action="publish_content",
                reason=(
                    "Crypto-launch content does not contain any acceptable AI "
                    "disclosure string. FTC Endorsement Guides (2023 update) "
                    "require that AI-generated promotional content be disclosed."
                ),
                remediation=(
                    "Append one of the acceptable disclosure variants to the "
                    "post: 'AI-generated', 'Posted by SINCOR Agent', 'Drafted "
                    "by AI', etc. Full list in AI_DISCLOSURE_VARIANTS."
                ),
            )
    ...
    def check_content(self, content: str, agent: str = "unknown", content_type: str = "general") -> None:
        ...
        if content_type in {"crypto_launch", "token_promotion", "sinc_launch",
                            "axiom_launch", "trading_post"}:
            self.check_crypto_promotion(content, agent=agent)
            self.check_ai_disclosure(content, agent=agent)
