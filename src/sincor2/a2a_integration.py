...
_A2A_STORE_MODE = os.getenv("A2A_TASK_STORE", "file").lower()
_A2A_STORE_PATH = Path(os.getenv("A2A_TASK_STORE_PATH", "data/a2a_tasks.json"))


def _serialize_task(task: A2ATask) -> dict[str, Any]:
    return {
        "id": task.id,
...
def _deserialize_task(data: dict[str, Any]) -> A2ATask:
    state_raw = data.get("state", "submitted")
    try:
    ...
    history = [
        A2AMessage(
            ...
            parts=m.get("parts", []),
            context_id=m.get("contextId", m.get("context_id", data.get("context_id", ""))),
            task_id=m.get("taskId", m.get("task_id", data.get("id", ""))),
        )
        for m in data.get("history", [])
    ]
    artifacts = [
        A2AArtifact(
            artifact_id=a.get("artifactId", a.get("artifact_id", "")),
            parts=a.get("parts", []),
            name=a.get("name"),
    ...
    return A2ATask(
        id=data["id"],
        context_id=data.get("context_id", data["id"]),
        ...
        axm_paid=int(data.get("axm_paid", 0)),
...
def _load_file_store() -> None:
    if _A2A_STORE_MODE != "file" or not _A2A_STORE_PATH.is_file():
        return
    try:
        payload = json.loads(_A2A_STORE_PATH.read_text(encoding="utf-8"))
        with _store_lock:
            for row in payload.get("tasks", []):
                task = _deserialize_task(row)
                _tasks[task.id] = task
        logger.info("[A2A] Loaded %s tasks from %s", len(_tasks), _A2A_STORE_PATH)
    except Exception as exc:
        logger.warning("[A2A] Could not load task file store: %s", exc)


def _persist_file_store() -> None:
    if _A2A_STORE_MODE != "file":
        return
    try:
        _A2A_STORE_PATH.parent.mkdir(parents=True, exist_ok=True)
        with _store_lock:
            payload = {"tasks": [_serialize_task(t) for t in _tasks.values()]}
        _A2A_STORE_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    except Exception as exc:
        logger.warning("[A2A] Could not persist task store: %s", exc)


_env = os.getenv("FLASK_ENV", "production").lower()
if _A2A_STORE_MODE == "memory" and _env not in _DEV_ENVS:
    logger.warning(
        "A2A task store is in-memory. Set A2A_TASK_STORE=file (default) or redis for persistence."
    )
_load_file_store()
