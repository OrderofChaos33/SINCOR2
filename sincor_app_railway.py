...
def login():
    ...
    if request.method == "POST":
        ...
        # Validate against environment variable credentials (set in production)
        correct_email = os.getenv("LOGIN_EMAIL")
        correct_password = os.getenv("LOGIN_PASSWORD")

        # Require both email and password to be set in environment
        if not correct_email or not correct_password:
            log("ERROR: LOGIN_EMAIL and LOGIN_PASSWORD not configured")
            return render_template('login.html', error="Authentication system not configured. Contact administrator.")
...
def health_check():
    """Health check for monitoring - admin only."""
...
def pricing():
    ...
    try:
        ...
        logger.error(f"Error loading pricing page: {e}")
...
def whitepaper():
    ...
    try:
        ...
        logger.error(f"Error loading whitepaper: {e}")
...
def buy():
    ...
    try:
        ...
        logger.error(f"Error loading buy page: {e}")
...
</head><body><p>Redirecting to login...</p></body></html>'''
    try:
