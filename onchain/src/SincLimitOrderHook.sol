...
///         This protects limit order users and improves execution quality for the SINCOR ecosystem.
contract SincLimitOrderHook is LimitOrderHook {
    ...
    /// @notice Normal fee: 0.30% (3000 = 0.30% in hundredths of a bip)
    uint24 public constant BASE_FEE = 3000;
    /// @notice Elevated fee applied to second+ swap in same block: 3.00%
    uint24 public constant SANDWICH_FEE = 30000;

    /// @notice Emitted when a swap triggers the elevated sandwich fee
    event SandwichFeeApplied(bytes32 indexed poolId, uint256 blockNumber, uint24 fee);
    ...
    function getHookPermissions() public pure override returns (Hooks.Permissions memory perms) {
        ...
        // (afterInitialize + afterSwap for limit order fulfillment).
        perms.beforeSwap = true;
    }

    /// @dev Counts swaps per pool per block and scales the fee accordingly.
    ///      Returns dynamic fee to the pool manager.
    function _beforeSwap(
        ...
        if (fee == SANDWICH_FEE) {
            emit SandwichFeeApplied(pid, block.number, fee);
        }
    ...
    /// @notice View helper: returns the fee that would apply to the next swap on this pool in the current block.
    function getCurrentFee(PoolKey calldata key) external view returns (uint24) {
        bytes32 pid = keccak256(abi.encode(key));
        uint256 count = swapsInBlock[pid][block.number];
        return count >= 1 ? SANDWICH_FEE : BASE_FEE;
    }
}
