...
_BPS_DENOM = 10_000
# 2026-09-02: settlement widened beyond AXM. A buyer agent that discovered
# SINCOR previously had to go acquire AXIOM before it could pay — the same
# friction fixed for the human checkout in 0582882. The accepted set now comes
# from the settlement-asset registry (SINC and unknown tokens still denied),
# while everything still *lands* in USDC per sincor2.settlement_policy.
def _allowed_a2a_assets() -> frozenset:
    """Accepted payment symbols, including the AXIOM alias."""
    from sincor2.a2a_settlement_assets import accepted_symbols
    return frozenset(set(accepted_symbols()) | {"AXIOM"})


# Kept as a module attribute for backward compatibility with existing callers.
ALLOWED_A2A_ASSETS = _allowed_a2a_assets()
...
class AgentSkill:
    ...
    free_quota: int = 0
    # Explicit USD anchor for this skill (0 = derive from AXM spot).
    # An explicit anchor keeps USDC/USDT/DAI quotes alive through an AXM spot
    # outage — the failure mode that broke the human checkout before 0582882.
    usd_price: float = 0.0
    ...
    def effective_usd_price(self) -> Optional[float]:
        """USD value of one call, or ``None`` when it cannot be established.

        Prefers the explicit ``usd_price`` anchor; otherwise derives it from the
        AXM price and live AXM spot. Returns ``None`` rather than guessing.
        """
        if self.usd_price and self.usd_price > 0:
            return float(self.usd_price)

        from sincor2 import a2a_settlement_assets as _sa

        spot = _sa.spot_usd("AXM")
        if not spot or spot <= 0:
            return None
        return (self.axm_price_wei / 10**18) * spot

    def payment_quotes(self) -> Dict[str, Any]:
        """Per-asset quotes for this skill. Empty when USD value is unknown."""
        from sincor2 import a2a_settlement_assets as _sa

        usd = self.effective_usd_price()
        if not usd or usd <= 0:
            return {}
        try:
            return _sa.quote_usd(usd)
        except ValueError:
            return {}

    def to_dict(self) -> Dict[str, Any]:
        from sincor2 import a2a_settlement_assets as _sa
        from sincor2.settlement_policy import SETTLEMENT_ASSET
        ...
        return {
            ...
            # --- multi-asset settlement (2026-09-02) ---
            "usdPrice":                 round(usd, 2) if usd else None,
            "acceptedAssets":           _sa.accepted_symbols(),
            "paymentQuotes":            self.payment_quotes(),
...
def _reject_unsupported_asset(token) -> Optional[str]:
    """Return an error string if *token* is not an accepted settlement asset.

    ``None``/empty means the caller did not name an asset, which is allowed —
    the payment gate then applies the default AXM price.
    """
    if not token or not str(token).strip():
        return None
    from sincor2.a2a_settlement_assets import accepted_symbols, is_accepted

    if not is_accepted(token):
        return (
            f"Unsupported settlement asset {token!r}. "
            f"Accepted: {', '.join(accepted_symbols())}."
        )
    return None


# Legacy alias — settlement is no longer AXM-only, but the name is still imported.
_reject_non_axm = _reject_unsupported_asset
...
def _handle_send(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    reject_msg = _reject_unsupported_asset(offered_token)
    if reject_msg:
