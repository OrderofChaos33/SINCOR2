...
contract MoebiusMEVHookTest is Test {
    ...
    function setUp() public {
        ...
        real = new MockERC20("Real Asset", "REAL", 18);
    ...
    /// @dev Accept native refund from seedLiquidity escrow.
    receive() external payable {}

    // ==================== NATIVE (ETH) REAL-ASSET PATH ====================

    function test_nativePool_holdMode_nativeBidSplit() public {
        // Native/pMEV pool: native (address 0) always sorts as currency0
        PoolKey memory nkey = PoolKey({
            currency0: Currency.wrap(address(0)),
            currency1: Currency.wrap(address(pToken)),
            fee: DYNAMIC_FEE,
            tickSpacing: TICK_SPACING,
            hooks: IHooks(address(hook))
        });
        manager.initialize(nkey, SQRT_PRICE_1_1);
        hook.setPoolSwapFee(nkey, POOL_FEE);

        // Central bank seeds with native real asset (escrow via msg.value, leftover refunded)
        ...
        hook.seedLiquidity{value: 120 ether}(nkey, -600, 600, liquidity, 120 ether, 200 ether);

        // Searcher escrows a NATIVE bid; hold mode needs no swaps
        uint256 bid = 1 ether;
        uint256 treasuryBefore = treasury.balance;
        vm.deal(address(searcher), bid);
        vm.prank(address(searcher));
        hook.executeMEV{value: bid}(nkey, FLASH, bid, abi.encode(uint8(0), nkey, FLASH, false));

        assertEq(treasury.balance - treasuryBefore, bid / 5, "treasury got native 20%");
        assertEq(address(hook).balance, 0, "hook retains no native");
        assertEq(pToken.balanceOf(address(searcher)), 0, "searcher holds no leftover pMEV");
    }

    function test_rescue_ownerOnly_paysTreasuryOnly() public {
        real.mint(address(hook), 1 ether);
        vm.prank(stranger);
        vm.expectRevert(MoebiusMEVHook.Unauthorized.selector);
        hook.rescue(Currency.wrap(address(real)), 1 ether);

        // Rescue is hard-wired to the governed treasury sink
        hook.rescue(Currency.wrap(address(real)), 1 ether);
        assertEq(real.balanceOf(treasury), 1 ether);
        assertEq(real.balanceOf(address(hook)), 0);
    }
}
