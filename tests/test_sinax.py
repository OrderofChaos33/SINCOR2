...
# ===========================================================================
# PTN — Proof Topology Navigator Tests
# ===========================================================================

_START = "⊢ ∀ n : ℕ, n + 0 = n"
_TARGET = "closed"
_LEMMA_1 = "⊢ 0 + n = n"
_LEMMA_2 = "⊢ n + succ m = succ (n + m)"
_FAILED_STATE = "⊢ False  -- dead end"
_ALL_STATES = [_START, _TARGET, _LEMMA_1, _LEMMA_2, _FAILED_STATE]


class TestProofManifold:
    def test_import(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        assert ProofManifold is not None

    def test_embed_returns_manifold_point(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        m = ProofManifold(dim=32)
        pt = m.embed(_START)
        assert pt.coordinates.shape == (32,)
        assert pt.source_state == _START
        assert isinstance(pt.curvature, float)

    def test_embed_different_states_differ(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        m = ProofManifold(dim=32)
        p1 = m.embed(_START)
        p2 = m.embed(_TARGET)
    ...
    def test_riemannian_distance_non_negative(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        m = ProofManifold(dim=32)
        p1 = m.embed(_START)
        p2 = m.embed(_LEMMA_1)
        assert m.riemannian_distance(p1, p2) >= 0

    def test_nearest_neighbours(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        m = ProofManifold(dim=32)
        points = [m.embed(s) for s in _ALL_STATES]
    ...
    def test_build_region(self):
        ...
        for s in _ALL_STATES:
            m.embed(s)
        region = m.build_region(_START, radius=5.0)
        assert region.centre.source_state == _START
        assert region.radius == 5.0

    def test_curvature_training_signal(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        m = ProofManifold(dim=32)
        signal = m.curvature_training_signal(_ALL_STATES)
        assert signal["num_states"] == len(_ALL_STATES)
        assert "mean_curvature" in signal
        assert "centroid" in signal


class TestGeodesicFlowEngine:
    def test_import(self):
    ...
    def test_geodesic_returns_path(self):
        ...
        path = engine.geodesic_flow(_START, _TARGET)
        assert path.start_state == _START
        assert path.target_state == _TARGET
        assert len(path.waypoints) >= 2
        assert path.path_length >= 0

    def test_geodesic_has_tactics(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        from sincor2.sinax.geodesic_flow import GeodesicFlowEngine, FlowConfig
        m = ProofManifold(dim=32)
        engine = GeodesicFlowEngine(manifold=m, config=FlowConfig(num_steps=5))
        path = engine.geodesic_flow(_START, _LEMMA_1)
        assert len(path.tactics) >= 2
        for t in path.tactics:
            assert isinstance(t, str) and len(t) > 0

    def test_teleport_when_close(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        from sincor2.sinax.geodesic_flow import GeodesicFlowEngine, FlowConfig
        m = ProofManifold(dim=32)
        engine = GeodesicFlowEngine(
            manifold=m,
            config=FlowConfig(teleport_threshold=999.0),
        )
        path = engine.geodesic_flow(_START, _START)
        assert path.teleported

    def test_proof_transfer_score(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        from sincor2.sinax.geodesic_flow import GeodesicFlowEngine, FlowConfig
        m = ProofManifold(dim=32)
        engine = GeodesicFlowEngine(manifold=m, config=FlowConfig(num_steps=5))
        path = engine.geodesic_flow(_START, _TARGET)
        score = engine.proof_transfer_score(path, _LEMMA_1)
        assert 0.0 <= score <= 1.0


class TestHomologyDetector:
    def test_import(self):
    ...
    def test_analyse_returns_report(self):
        ...
        report = hd.analyse(_ALL_STATES)
        assert report.num_states == len(_ALL_STATES)
    ...
    def test_detect_dead_ends(self):
        ...
        m = ProofManifold(dim=32)
        hd = HomologyDetector(manifold=m)
        dead_ends = hd.detect_dead_ends([_FAILED_STATE, _FAILED_STATE + " 2"])
...
class TestMorseFilter:
    ...
    def test_decompose_returns_decomposition(self):
        ...
        decomp = mf.decompose(_ALL_STATES)
        assert len(decomp.critical_points) == len(_ALL_STATES)
    ...
    def test_simplification_power_in_range(self):
        from sincor2.sinax.proof_manifold import ProofManifold
        from sincor2.sinax.morse_filter import MorseFilter
        m = ProofManifold(dim=32)
        mf = MorseFilter(manifold=m)
...
class TestAxiomSolver:
    ...
    def test_solve_returns_result(self):
        ...
        result = solver.solve(_START, _TARGET)
        assert result.start_state == _START
        assert result.target_state == _TARGET
    ...
    def test_solve_batch(self):
        ...
        problems = [
            ...
            {"start": _LEMMA_1, "target": _LEMMA_2},
        ]
        results = solver.solve_batch(problems)
        assert len(results) == 2

    def test_training_signal(self):
        from sincor2.sinax.axiom_solver import AxiomSolver
        solver = AxiomSolver(manifold_dim=32)
        signal = solver.get_manifold_training_signal(_ALL_STATES)
        assert "curvature" in signal
        assert "betti_numbers" in signal


class TestProofTopologyNavigator:
    def test_import(self):
    ...
    def test_solve(self):
        ...
        result = nav.solve(_START, _TARGET)
        d = result.to_dict()
    ...
    def test_geodesic(self):
        from sincor2.sinax.ptn import ProofTopologyNavigator
        nav = ProofTopologyNavigator(manifold_dim=32, num_flow_steps=5)
        result = nav.geodesic(_START, _TARGET)
        assert "tactics" in result
    ...
    def test_homology(self):
        from sincor2.sinax.ptn import ProofTopologyNavigator
        nav = ProofTopologyNavigator(manifold_dim=32)
        result = nav.homology(_ALL_STATES)
    ...
    def test_training_signal(self):
        from sincor2.sinax.ptn import ProofTopologyNavigator
        nav = ProofTopologyNavigator(manifold_dim=32)
        result = nav.training_signal(_ALL_STATES)
    ...
    def test_transfer_score(self):
        from sincor2.sinax.ptn import ProofTopologyNavigator
        nav = ProofTopologyNavigator(manifold_dim=32, num_flow_steps=5)
        r1 = nav.solve(_START, _TARGET)
        score = nav.transfer_score(r1, _LEMMA_1)
        assert 0.0 <= score <= 1.0
