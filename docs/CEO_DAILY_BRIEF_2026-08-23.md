...
**Treasury (`0x09E289…12Ac`) live snapshot (Basescan multichain, 23 Aug 2026 ~09:00 CDT):**
- Base USDC: 199.4164 (~$199.40)
- Base ETH: 0.00447776 (~$11.00 @ ~$2459)
- Polygon USDC.e: ~$11.06 + POL residual ~$2.49
...
Prior material tracking remains open until evidence of realized inflow or all P0 landed. Do not close on theater. Tracking issue lineage: #168 and related.
...
**Yield Aggregator — Plan against live $229.14 (risk_budget=0.30)**
...
### P0
1. Re-run `YieldAggregator.plan_rebalance(~229.14, risk_budget=0.30)` (`src/sincor2/defi/yield_aggregator.py`). Wire plan + `simulate_year_pnl` into TOA ingest this cycle. Scheduler must pull live capital (no hardcoded values). Confirm shared_liq_vault correctly gated at current capital.
...
**— CEO / TOA Oversight**  
23 Aug 2026  
Canonical addresses unchanged. Material actions remain open until realized inflow or P0 evidence. Tracking issue to be opened/updated for this cycle.
