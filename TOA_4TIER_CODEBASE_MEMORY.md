...
**Created:** 2026-08-18  
**Last Corrected:** 2026-08-18 20:45 CDT  
**Purpose:** Permanent, hierarchical memory attached to the SINCOR2 GitHub repository that forces every future change (human or agent) to stay inside the AXM-primary token scope defined by CEO directive 2026-08-16 (address corrected same day). Prevents regression to SINC-primary paths, dual-token complexity, or unapproved expansion.
...
7. The only valid primary AXM address is `0x4c3fb66f14fbaa2088c9ae91017ba770da53715a`.

## Tier 2 — Operational Memory (updated after every successful change)
- 2026-08-18 20:45: **CRITICAL CORRECTION** — AXM primary address corrected from stale `0xfF7aF6...` to live `0x4c3fb66f14fbaa2088c9ae91017ba770da53715a` across CANONICAL_ADDRESSES.md, settlement.py, and this memory. Previous address was a dead PumpClawToken.
- 2026-08-18 earlier: settlement.py flipped to AXIOM primary + full address/amount/symbol validation + init-time canonical checks.
- Next required actions (ordered):
  1. settings.py — add axiom_contract_address field pointing to new address, keep sinc only for legacy reads.
  ...
  8. .env.example and config/railway_add_these.env — AXIOM_CONTRACT_ADDRESS first with new address.
  9. onchain scripts that still hard-code SINC as primary for new deploys must be annotated legacy.
  10. **AIRDROP PREP** — Snapshot ~2600 SINC holders and prepare distribution plan against new AXM contract.
...
5. Memory itself is updated only by appending dated entries under Tier 2; Tier 0 and Tier 1 are append-only under Founder signature.
...
**Last verified:** 2026-08-18 20:45 CDT by Grok + TOA protocol (address correction).
